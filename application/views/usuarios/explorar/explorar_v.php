...
<?php    
    //Clases botones acción
        ...
        if ( $this->session->userdata('role') <= 1 ) { $clases_btn['exportar'] = ''; }
?>

<?php $this->load->view("{$carpeta_vistas}explorar/script_js"); ?>
<?php $this->load->view($vista_menu) ?>

<div class="row">
    <div class="col-md-6">
        <?php $this->load->view("{$carpeta_vistas}explorar/form_busqueda_v"); ?>
    </div>
    ...
    <div class="col-md-3">
        ...
        <div class="btn-group hidden-xs <?= $clases_btn['exportar'] ?>" role="group">
            <?= anchor("{$controlador}/exportar/?{$busqueda_str}", '<i class="fa fa-download"></i> Exportar', 'class="btn btn-success" title="Exportar los ' . $cant_resultados . ' registros a archivo de Excel"') ?>
        </div>
    </div>
...
<?php $this->load->view("{$carpeta_vistas}explorar/tabla_v"); ?>