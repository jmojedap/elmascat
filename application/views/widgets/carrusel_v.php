        ...
        <?php foreach ($imagenes->result() as $row_imagen) : ?>
            ...
            <div class="item <?= $clase_item ?>">
                ...
                <div class="carousel-caption">
                    <h1><?= $row_imagen->titulo ?></h1>
                    <p><?= $row_imagen->descripcion ?></p>
                    <?php if (strlen($row_imagen->link)) { ?>
                        <?= anchor($this->Pcrn->preparar_url($row_imagen->link), 'Ver', 'class="btn btn-default w3"') ?>
                    <?php } ?>
                </div>
            </div>
        <?php endforeach ?>
