...
<div id="product_books" class="center_box_750">
    ...
    <div class="table-responsive">
        <table class="table bg-white bg-blanco">
            <thead>
                <th width="120px"></th>
                <th>Descripción</th>
                <?php if ( $this->session->userdata('role') <= 10 ) : ?>
...
<script>
    new Vue({
        ...
        methods: {
            add_post: function(){
                axios.get(this.url_app + 'productos/add_post/' + this.product_id + '/' + this.book_id)
                .then(response => {
                    console.log(response.data)
                    window.location = this.url_app + 'productos/books/' + this.product_id;
                })
            ...
            remove_post: function(book_id, meta_id){
                axios.get(this.url_app + 'productos/remove_post/' + this.product_id + '/' + meta_id)
                .then(response => {
                    console.log(response.data)
                    window.location = this.url_app + 'productos/books/' + this.product_id;
                })
